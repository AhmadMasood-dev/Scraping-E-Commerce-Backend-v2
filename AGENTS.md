...
GET /api/v1/search?query=&city=&lang=&description=
 NLP → LRU cache → DISCOVERY (search API + direct sources, parallel) → domain allowlist filter
...
- **SSRF guard:** only fetch https + allow-listed PK shopping domains; never private IPs/localhost.
...
src/
  ...
  domainFilter.js   PK-shopping allowlist + SSRF guard
